function showFeed(data) {
  ...
  for (var i = 0; i < Math.min(entries.length, 3); i++) {
    ...
    var desc = tempDiv.textContent.trim().substring(0, 100) + '...';

    // Ambil paragraf pertama dari konten artikel
let firstParagraph = fullContent.match(/<p>(.*?)<\/p>/i);
let excerpt = firstParagraph ? firstParagraph[1] : '';
...
html += '<div style="background:#f5f5f5;padding:10px;border-radius:8px">' +
    ...
    '<p style="margin:0;font-size:14px;color:#444">' + excerpt + '</p>' +
    '</a></div>';
  }
  html += '</div>';
